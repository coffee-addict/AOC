...
//    REP(j,cds0.size()) res.epb(mkt(cds0[j][0],cds0[j][1],cds0[j][2]));
  ...
  int md = 0;
  REP(i,n) REP(j,n) {
    int dx0 = dxyz20[i][0], dy0 = dxyz20[i][1], dz0 = dxyz20[i][2];
    int dx1 = dxyz20[j][0], dy1 = dxyz20[j][1], dz1 = dxyz20[j][2];
    int tmp = abs(dx0-dx1) + abs(dy0-dy1) + abs(dz0-dz1);
    md = max(md, tmp);
  }
  cout << "max manhattan distance = " << md << endl; 
