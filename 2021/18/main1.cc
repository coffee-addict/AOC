...
int main() {
  ...
  while (cases--) {
    int n; cin >> n; 
    string s; cin >> s; 
    vector<TIII> v =  parse(s);
    REP(i,n-1) {
      REP(j,v.size()) v[j] = mkt(nth(v[j],0)+1, nth(v[j],1), nth(v[j],2));
      cin >> s;
      vector<TIII> w =  parse(s);
      REP(j,w.size()) v.epb(mkt(nth(w[j],0)+1, nth(w[j],1), nth(w[j],2)));
      v = explode(v);
      v = split(v);
    }
for (TIII x : v) {
